...
- [x] 1.1 ブロックリストからルールを組み立て、入れ直す関数を作る
...
- [x] 1.2 manifest の権限を設定し、インストール時にルールを登録する
  ...
  - _要件: 3.1, 4.1, 5.1_
  - _範囲: `apps/extension/wxt.config.ts`, `apps/extension/entrypoints/background.ts`, `apps/extension/package.json`, `pnpm-lock.yaml`（`web-ext`。WXT がブラウザを起動するのに要る）_
  - _並行: 不可（`package.json` を 2.1 と共有する）_

- [x] 2. E2E

- [x] 2.1 拡張を読み込んだ Chromium で E2E を走らせる基盤を作る
  ...
  - _範囲: `apps/extension/package.json`, `apps/extension/playwright.config.ts`, `apps/extension/tsconfig.json`（e2e を外す）, `apps/extension/e2e/`, `pnpm-lock.yaml`_
  - _並行: 不可（`pnpm-lock.yaml` を更新する。2.2 以降がこの基盤を使う）_

- [x] 2.2 ブロックする範囲としない範囲を確かめる
...
- [x] 2.3 元URLを欠かさず渡すことを確かめ、テストが壊れを検出することを確かめる
...
- [x] 2.4 再起動をまたいでもルールが保たれ、重複しないことを確かめる
