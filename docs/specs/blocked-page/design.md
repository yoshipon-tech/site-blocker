...
| ---- | ---- | ---------- | ---------------- |
| ブロック画面のエントリを `apps/web/blocked/index.html` に置き、Vite の `build.rollupOptions.input` で指定する。ルートの `index.html` は置かない | 出力が `dist/blocked/index.html` になり、Pages が `/site-blocker/blocked/` を 301 なしで返す（4.2）。ルートを占有しない（契約） | SPA + ルーター: 1画面のために依存が増え、ルートも占有する / `404.html` へのフォールバック: ステータスが 404 になる / ハッシュルーティング: フラグメントは元URLに使うので衝突する | パスは配布済み拡張に焼き込まれる。変えると古い拡張が壊れる（`guide-tech.md`） |
...
| フラグメントの解釈を `parseBlockedUrl(hash)` の純粋関数にし、`decodeURI` でデコードする | `decodeURIComponent` だと `%26` `%2F` まで戻り、URL の区切りが変わって見える（1.7）。純粋関数なら DOM なしで境界値をテストできる | `decodeURIComponent`: 上記 / デコードしない: U2 で却下 | 表示の仕様だけに効く。拡張側への影響はない |
...
| 公開は `actions/configure-pages` → `actions/upload-pages-artifact` → `actions/deploy-pages` の公式手順で行う | ブランチに成果物をコミットしないで済む。ビルドとデプロイのジョブを分け、ビルドが失敗したらデプロイジョブが走らない（4.4） | `gh-pages` ブランチへ push: 成果物が履歴に残り、書き込み権限が要る | リポジトリの Pages 設定（Source: GitHub Actions）とセットで効く |
| ワークフローは `push`（main、`apps/web/**` などのパス指定）と `workflow_dispatch` で起動する | 画面に関係ない変更で再デプロイしない（4.1）。手動でも公開できる（4.5） | main への push すべて: 拡張だけの変更でも公開が走る | 依存するファイルを増やしたらパス指定も増やす |
...
| `vite.config.ts` | 変更 | `base: "/site-blocker/"`、`input` に `blocked/index.html` |
...
| `e2e/blocked.spec.ts`, `playwright.config.ts` | 新規 | `vite preview` で配信したビルド結果に対する E2E |
...
| hash | 1行目 | 元URL欄 |
| ---- | ----- | ------- |
| 空、または `#` のみ | 「このページは site-blocker 拡張がブロックしたときに開く画面です」 | 出さない |
| ホスト名のある URL | 「**<ホスト名>** は今ブロックしています」 | デコード済みの元URL（リンクにしない、1.4） |
...
sequenceDiagram
  ...
  Build->>Build: test → build → playwright install → test:e2e
  alt いずれかが失敗
...
- 権限は `contents: read`、`pages: write`、`id-token: write` のみ。`concurrency: pages`、`cancel-in-progress: false` で公開を途中で打ち切らない
...
| 1.7 / 1.9 | コマンド | 単体テスト: 日本語がデコードされる、`%26` は残る、不正な `%` は届いたまま。URL として読めないときホスト名が `null` |
| 1.8 | Playwright | 別ページから遷移して開くとボタンが出て、押すと元のページに戻る。直接開くと出ない |
| 2.1 | Playwright | hash なし・`#` のみで説明文が出て、元URL欄がない |
| 3.1 / 3.2 | Playwright | 読み込み中の全リクエストを記録し、すべて同一オリジンで、どれも元URLの文字列を含まない |
| 4.1 / 4.5 | ブラウザ | main へのマージ後と手動実行後に Actions が成功し、公開 URL が開ける |
...
| 4.4 | ブラウザ | ワークフローのジョブ依存（`needs: build`）をレビューで確認する。故意に失敗させる検証は行わない |
...
- 使う Actions（`checkout` / `configure-pages` / `upload-pages-artifact` / `deploy-pages` / `mise-action`）のメジャーバージョンは、実装時に最新のリリースを確認して固定する
