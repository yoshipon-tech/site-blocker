...
| `lint` は `eslint . --max-warnings 0` で実行し、警告も失敗にする | react-hooks の `exhaustive-deps` は既定で警告なので、警告を許すと依存の漏れが終了コード 0 で通る（2.3）。実装時に判明 | 該当ルールだけ `error` に上げる: 他の警告ルールで同じ穴が残る | 警告を許すと、依存の漏れを見逃す |
