...
describe("parseBlockedUrl", () => {
  ...
  it("元URLに含まれる # 以降も元URLとして残す", () => {
    expect(parseBlockedUrl("#https://example.com/a#b")?.url).toBe(
      "https://example.com/a#b",
    );
  });

  it("ホスト名の先頭の www. を落とす", () => {
    expect(parseBlockedUrl("#https://www.youtube.com/watch?v=1")?.host).toBe(
      "youtube.com",
    );
  });
  ...
  it("URL として読めないときはホスト名を null にする", () => {
    expect(parseBlockedUrl("#not a url")).toEqual({
      url: "not a url",
      host: null,
    });
  });
